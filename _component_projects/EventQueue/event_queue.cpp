...
namespace EventQueue
{
    ...
    EventQueue::~EventQueue()
    {
        // queue a null pointer to signal the thread to terminate
        {
            ...
            lthread::lock_guard<lthread::mutex> _lock(_eventQueueLock);
            events.push( nullptr );
    ...
    EventQueue::hookListener( Listener * l )
    {
        ...
        {
            // lock
            lthread::lock_guard<lthread::mutex> _lock(_listenerVectorLock);
    ...
    EventQueue::_eventProcessingFunction( void * _data )
    {
        ...
        while( true )
        {
            ...
            {
                // lock
                lthread::lock_guard<lthread::mutex> _lock(eventQueue->_listenerVectorLock);
                ...
                {
                    // multi threaded
                    lthread::thread ** listenerThreads = new lthread::thread*[num_threads-1];

                    for( std::size_t i = 0; i < num_threads-1; ++i )
                    {
                        listenerThreads[i] = new lthread::thread( [=]{ eventQueue->_passEventToListeners( e, i, num_threads ); } );
                    }
