...
namespace alloc
{
    class Allocator
    {
        protected:
            ...
            std::size_t _min_free_memory;
            std::size_t _max_num_allocations;

        public:

            Allocator( std::size_t block_size, void * block_start )
            : _block_start(block_start), _block_size(block_size), _used_memory(0), _num_allocations(0), _max_used_memory(0), _min_free_memory(block_size), _max_num_allocations(0)
            { }
            ...
            template < class Type >
            inline void deallocate( Type * block ) final { if( std::is_destructible<Type>::value ) { ((Type*)block)->~Type(); } this->deallocateBlock( block ); }
            ...
            virtual inline std::size_t minFreeMemory()     const { return _min_free_memory; }
        ...
        protected:
            inline void _incrementAllocations( std::size_t size ) { assert( size > 0 ); _max_num_allocations = std::max( _max_num_allocations, ++_num_allocations ); _min_free_memory = std::min( _min_free_memory, unusedMemory() ); _max_used_memory = std::max( _max_used_memory, _used_memory += size ); }
            inline void _decrementAllocations( std::size_t size ) { assert( size > 0 ); assert( _num_allocations > 0 ); --_num_allocations; _used_memory -= size; }
        private:
